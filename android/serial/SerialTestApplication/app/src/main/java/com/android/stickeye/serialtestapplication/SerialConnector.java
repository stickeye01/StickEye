...
import com.android.stickeye.serialtestapplication.Constants;
import com.android.stickeye.serialtestapplication.SerialCommand;
...
public class SerialConnector {
    ...
    public SerialConnector(Context c, MainActivity.SerialListener l, Handler h) {
        mContext = c;
        mListener = l;
        mHandler = h;
    }
     ...
     ******************************************************/
    //Serial로 연결된 Device와 데이터를 주고 받는 쓰레드
    public class SerialMonitorThread extends Thread {
        ...
        public void run()
        {
            ...
            final char mStartCharacter = 's';
            final char mEndCharacter = 'e';
            while(!Thread.interrupted())
            {
                if(mPort != null) {
                    ...
                    try {
                        ...
                        Message msg = mHandler.obtainMessage(Constants.MSG_SERIAL_ERROR, 0, 0, "Error # run: " + e.toString()+" "+e.getStackTrace() + "\n");
                        mHandler.sendMessage(msg);
                        mKillSign = true;
                    }
                }
