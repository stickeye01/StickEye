...
// longest Hypo, Bottom 값이 이상한 값이 나왔을 때 전의 정상적인 값을 저장하여 오류났을때 사용
float flag_longhypo=50;
float flag_bottomhypo=15;
...
void startObstacDetect() {
  ...
  currentTime = millis();
    ...
    if(longestHypo  < 49 || longestHypo > 100)
        longestHypo=flag_longhypo;
    else
        flag_longhypo=longestHypo;
    
    printStr("이주형 : longestHypo", longestHypo);
...
float getTilt(float longestHypo){
   ...
   if(height1 > 35)
      height1=flag_bottomhypo;
   else
      flag_bottomhypo=height1;
